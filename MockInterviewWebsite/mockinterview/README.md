...
```bash
conda create python=3.8 -n mockinterview
```
2 - <a name="step-2">Activate the conda environment</a>

```bash
conda activate mockinterview
```
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///mockinterview` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE mockinterview WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE mockinterview TO ;
```
...
ERROR:  database "mockinterview" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists mockinterview
...
```bash
dropdb --if-exists mockinterview
